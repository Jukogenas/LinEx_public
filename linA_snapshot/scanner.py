...
GENO_TYPES = ["matrix", "vector", "scalar"]

GENO_KEYWORDS = ["min", "parameters", "variables", "st", "parameter", "variable", "minimize"]

ATTRIBUTE = ["psd", "symmetric", "sparse"]
    ...
    def getSym(self):
        ...
        # detect variables or functions
            ...
            while self.current in ALPHA + DIGIT:
                ...
                self.current = self.input.next()
            if self.geno:
                if identifier.lower() in GENO_TYPES:
                    identifier = identifier.lower()
                    description = "type"
                if identifier.lower() in ATTRIBUTE:
                    identifier = identifier.lower()
                    description = "attribute"

            # if ident is function name
            while self.current == " ":
                self.current = self.input.next()
            if self.current == "(":
        ...
        elif self.current in COMPARE.keys() and self.geno:
            description = "compare"
            identifier = self.current
            if identifier in ["<", ">", "="]:
                self.current = self.input.next()
